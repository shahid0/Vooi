...
class VooiDC:
    ...
    def handle_autotrade(self):
        ...
        if not autotrade_data:
            ...
            if autotrade_data:
                ...
                self.log("Unable to start a new autotrade.", 'error')
                return

        if autotrade_data['status'] == 'finished':
            self.log("Autotrade finished. Claiming rewards...", 'success')
            claim_result = self.claim_autotrade(autotrade_data['autoTradeId'])
            if claim_result:
                self.log(f"Successfully claimed autotrade. Earned {claim_result['reward']['virtMoney']} USD {claim_result['reward']['virtPoints']} VT", 'success')
                self.log(f"Total balance {claim_result['balance']['virt_money']} USDT | {claim_result['balance']['virt_points']} VT", 'success')
            else:
                self.log("Unable to claim autotrade rewards.", 'error')
    ...
    def start_tapping_session(self):
        ...
        try:
            ...
            self.log(f"Error starting tapping session: {str(e)}", 'error')
            return None
    ...
    def play_tapping_game(self):
        for game_number in range(1, 6):
            ...
            if not session_data:
                self.log(f"Unable to start game {game_number}. Skipping this game.", 'warning')
                continue

            virt_money_limit = int(session_data['config']['virtMoneyLimit'])
            virt_points_limit = int(session_data['config']['virtPointsLimit'])

            self.log(f"Waiting 30 seconds to complete the tap game...", 'custom')
            time.sleep(30)

            virt_money = random.randint(max(1, int(virt_money_limit * 0.85)), int(virt_money_limit * 1))
            ...
            if result:
                self.log(f"Tap successful, earned {result['tapped']['virtMoney']} USD | {result['tapped']['virtPoints']} VT", 'success')
            else:
                self.log(f"Unable to complete game {game_number}", 'error')
    ...
    def manage_tasks(self):
        ...
        for task in new_tasks:
            ...
            if result and result['status'] == 'in_progress':
                ...
                self.log(f"Unable to start task {task['description']}", 'error')
        ...
        for task in completed_tasks:
            ...
            if result and 'claimed' in result:
                ...
                self.log(f"Task {task['description']} completed successfully | Reward {virt_money} USD | {virt_points} VT", 'success')
            else:
                self.log(f"Unable to claim reward for task {task['description']}", 'error')
